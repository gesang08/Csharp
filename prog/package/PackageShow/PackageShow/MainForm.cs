...
namespace PackageShow
{
    public partial class PackageShowFormClass : Form
    {
        ...
        int total_layer = 0, total_plate = 0;
        float total_area = 0;
        ...
        int Long = 0, Short = 0;
        string part_id = null;
        bool right_flag = false;//右键按下标志
        bool left_flag = false;//左键按下标志
        int line_offset = 3;//线宽为6时的偏移量
        /// <summary>
        /// 数据库变量声明
        /// </summary>
        ...
        public PackageShowFormClass()
        {
            ...
            this.PartInfoDataGridView.ClearSelection();//清除默认选择的cell
            init_componet_postion();//窗体控件布局与自定调整方法
            this.TopMost = true;//设置winFrom为顶级窗口，消除程序运行时的任务栏
        ...
        private void info_package_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            //this.TopMost = true;//设置winFrom为顶级窗口，消除程序运行时的任务栏
            //ShowWindow(FindWindow("Shell_TrayWnd", null), SW_HIDE);
            box_bed.DrawRectangle(new Pen(new SolidBrush(Color.FromArgb(255, 199, 237, 204)), 2*line_offset), box_x, box_y, box_height, box_width);
            ...
            else
            {
                if (!error_flag)
                {
                    ...
                    total_layer = Convert.ToInt32(info_package.Rows[current_row_index].Cells[1].Value);
        ...
        private void PackageShowFormClass_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.Right://显示下一层
                    box_bed.DrawRectangle(new Pen(new SolidBrush(Color.FromArgb(255, 199, 237, 204)), 6), box_x, box_y, box_height, box_width);
                    right_flag = true;
                    left_flag = false;
        ...
        public void get_info_package_from_db()
        {
            ...
            String PackageSql = "SELECT `Ap_id`,`Total_plies`,`Total_area`,`Package_num` FROM `work_package_task_list` WHERE `State` = 0";
            ...
            try
            {
                ...
                if (InfoPackageRead.HasRows)
                {
                    while (InfoPackageRead.Read())
                    {
                        ...
                        total_layer = InfoPackageRead.GetInt32(1);
                        total_area = InfoPackageRead.GetFloat(2);
                        total_plate = InfoPackageRead.GetInt32(3);
                ...
                if (PartInfoReader.HasRows)
                {
                    while (PartInfoReader.Read())
                    {
                        ...
                        for (int i = 0; i < PartInfoDataGridView.ColumnCount; i++)//把包信息显示到DataGridView里面
                        {
                            ...
                            this.PartInfoDataGridView.Rows[0].Cells[0].Selected = false;//取消默认选择的第一个cell
                        }
                    }
                }
            }
        ...
        public void set_selected_part_background()
        {
            for (int i = 0; i < PartInfoDataGridView.RowCount; i++)
            {
                if (PartInfoDataGridView.Rows[i].Cells[1].Value.ToString() == part_id)
                {
                    if (right_flag)
                    {PartInfoDataGridView.Rows[i].DefaultCellStyle.BackColor = Color.Yellow;}
                    if (left_flag)
                    { PartInfoDataGridView.Rows[i].DefaultCellStyle.BackColor = Color.FromArgb(255, 199, 237, 204); }
        ...
        public void show_first_layer()
        {
            ...
            float temp = 0;
            for (int plate = 0; plate < each_layer_total_plates[current_layer - 1]; plate++)//循环第一层每块
            {
                ...
                part_id = first_layer_info.Split('&')[5];
                ...
                set_selected_part_background();
                box_bed.DrawRectangle(new Pen(new SolidBrush(Color.Black), 2*line_offset), box_x + x / width_scale_ratio + line_offset, box_y + y / height_scale_ratio + line_offset, height / width_scale_ratio, width / height_scale_ratio);
        ...
        public void show_next_layer()
        {
            ...
            {
                next_layer_info = info_position[plate + (current_layer - 1) * 10];
                ...
                part_id = next_layer_info.Split('&')[5];
                if (is_change == 1)//height对应windows_width
                {
                    temp = height;
                    height = width;
                ...
                set_selected_part_background();
                box_bed.DrawRectangle(new Pen(new SolidBrush(Color.Black), 2*line_offset), box_x + x / width_scale_ratio, box_y + y / height_scale_ratio, height / width_scale_ratio-line_offset, width / height_scale_ratio
        ...
        public void show_before_layer()
        {
            ...
            string next_layer_info = null;
            float temp = 0;
            for (int plate = 0; plate < each_layer_total_plates[current_layer - 1]; plate++)//循环一层下的每块信息
            {
                next_layer_info = info_position[plate + (current_layer-1)*10];
                x = Convert.ToSingle(next_layer_info.Split('&')[0]);
                y = Convert.ToSingle(next_layer_info.Split('&')[1]);
                ...
                width = Convert.ToSingle(next_layer_info.Split('&')[3]);
                is_change = Convert.ToInt32(next_layer_info.Split('&')[4]);
                if (info_position[plate + current_layer * 10] != null)
                { part_id = info_position[plate + current_layer * 10].Split('&')[5]; }
                if (is_change == 1)//height对应windows_width
                {
                    temp = height;
                    height = width;
                ...
                set_selected_part_background();
                box_bed.DrawRectangle(new Pen(new SolidBrush(Color.Black), 2*line_offset), box_x + x / width_scale_ratio, box_y + y / height_scale_ratio, height / width_scale_ratio-line_offset, width / height_scale_ratio-line_offset);
                Font myFont = new Font("宋体", change_font_size(Math.Min(width, height)), FontStyle.Bold);
                StringFormat str_format = new StringFormat();
                str_format.Alignment = StringAlignment.Center;
                str_format.LineAlignment = StringAlignment.Center;
        ...
        public void draw_position(int box_long, int box_short, List<int> each_layer_total_plates, List<string> info_position)
        {
            ...
            for (int layer = 0; layer < total_layer; layer++)//循环每层
            {
                for (int plate = 0; plate < each_layer_total_plates[layer]; plate++)//循环每层下每块
                {
                    ...
                    box_bed.DrawRectangle(new Pen(new SolidBrush(Color.Black), 6), box_x + x, box_y + y, height, width);
        ...
        public void draw_main_frame()
        {
            ...
            box_bed.DrawRectangle(new Pen(new SolidBrush(Color.Black), 6), box_x, box_y, Long/width_scale_ratio, Short/height_scale_ratio);//把包的范围显示出来
        }
